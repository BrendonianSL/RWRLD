...
export default function Photo({
  ...
  return (
    <div
      className={`flex ${flip ? "flex-row-reverse" : "flex-row"
    ...
    >
      ...
      >
        ...
        <div className="mt-4 gap-4 flex items-center justify-center w-full text-black">
          ...
          <span className="text-[20px] block">{props.location}</span>
        </div>
      </motion.figure>

      <motion.div initial={{ opacity: 0, scale: 0 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0 }} className="text-black flex flex-col justify-center w-full relative">
        {props.aside && (
